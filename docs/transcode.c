...
// Build FFmpeg argument list based on backend, codec, audio settings, bitrate, and output type
...
char **build_ffmpeg_args(TranscodeBackend backend, TranscodeCodec codec, int surround51, int bitrate_kbps, TranscodeOutput output_type, const char *output_dest, int *argc_out) {
    // Max 80 args for all options (increased for HLS)
    char **argv = malloc(sizeof(char*) * 80);
    int argc = 0;
    
    // Static buffers for bitrate strings (persist for lifetime of args)
    static char video_bitrate[32];
    static char maxrate[32];
    static char bufsize[32];
        ...
        case BACKEND_SOFTWARE:
            switch (codec) {
                case CODEC_H264: 
                    ...
                    argv[argc++] = "veryfast";
                    if (bitrate_kbps > 0) {
                        snprintf(video_bitrate, sizeof(video_bitrate), "%dk", bitrate_kbps);
                        snprintf(maxrate, sizeof(maxrate), "%dk", bitrate_kbps);
                        snprintf(bufsize, sizeof(bufsize), "%dk", bitrate_kbps * 2);
                        argv[argc++] = "-b:v";
                        argv[argc++] = video_bitrate;
                        argv[argc++] = "-maxrate";
                        argv[argc++] = maxrate;
                        argv[argc++] = "-bufsize";
                        argv[argc++] = bufsize;
    ...
    // Use lower audio bitrate for low-bitrate modes
    const char *default_audio_bitrate = (bitrate_kbps > 0 && bitrate_kbps <= 1000) ? "64k" : "128k";
    const char *default_surround_bitrate = (bitrate_kbps > 0 && bitrate_kbps <= 1000) ? "128k" : "256k";
    const char *default_aac_surround_bitrate = (bitrate_kbps > 0 && bitrate_kbps <= 1000) ? "192k" : "384k";
    
    // HLS output setup
    if (output_type == OUTPUT_HLS) {
        // HLS always uses MPEG-TS or fMP4 segments. Using MPEG-TS is safer for broad compatibility with h264/hevc.
        // For AV1, fMP4/WebM segments might be needed, but mpegts is standard for HLS.
        // Let's force mpegts container for HLS segments regardless of codec for simplicity (except AV1 might be tricky).
        
        // Setup HLS flags
        argv[argc++] = "-f";
        argv[argc++] = "hls";
        argv[argc++] = "-hls_time";
        argv[argc++] = "4"; // 4s segments
        argv[argc++] = "-hls_list_size";
        argv[argc++] = "6"; // Keep 6 segments in playlist (24s live window)
        argv[argc++] = "-hls_flags";
        argv[argc++] = "delete_segments+append_list"; // clean up old files
        argv[argc++] = "-hls_segment_type";
        argv[argc++] = "mpegts"; 
        
        // For HLS, we need to handle audio codecs carefully. AAC is safe.
        if (codec == CODEC_AV1) {
             // AV1 in MPEG-TS is experimental in some versions, but let's try.
             // If not, we might need fmp4 segments.
             // For now, assume AAC audio + video.
             if (surround51) {
                argv[argc++] = "-af";
    ...
    if (output_type == OUTPUT_HLS && output_dest) {
        argv[argc++] = (char*)output_dest;
    } else {
        argv[argc++] = "pipe:1";
    }
...
int handle_transcode(int sockfd, TranscodeBackend backend, TranscodeCodec codec, const char *channel, int surround51, int bitrate_kbps) {
    ...
    if (ffmpeg_pid == 0) {
        ...
        int argc;
        char **argv = build_ffmpeg_args(backend, codec, surround51, bitrate_kbps, OUTPUT_PIPE, NULL, &argc);
        execvp("ffmpeg", argv);
        exit(1);
    }
...
void handle_transcode_m3u(int sockfd, const char *host, const char *backend, const char *codec, int bitrate_kbps, int surround51) {
    ...
    char opts[64] = {0};
    if (bitrate_kbps > 0) {
        snprintf(opts + strlen(opts), sizeof(opts) - strlen(opts), "/b%d", bitrate_kbps);
    }
    if (surround51) {
        strncat(opts, "/ac6", sizeof(opts) - strlen(opts) - 1);
    }
    ...
    for (int i = 0; i < channel_count; i++) {
        ...
        snprintf(buf, sizeof(buf), 
            ...
            "http://%s/transcode/%s/%s/%s%s\n",
            channels[i].number, channels[i].name, channels[i].number, channels[i].name,
            display_host, backend, codec, channels[i].number, opts);
        APPEND_M3U(buf);
    }
