...
static void fetch_and_process(const char *zip, const char *scan_file) {
    ...
    if (count == 0) {
        printf("[SCANNER] Generating full US ATSC scan list...\n");
        for (int i = 2; i <= 36; i++) found_channels[i] = 1;
        count = 35;
    } else {
            ...
            for (int i = 0; i < num_adapters; i++) {
                if (workers[i].pid > 0 && FD_ISSET(workers[i].pipe_fd, &fds)) {
                    ...
                    if (n > 0) {
                        ...
                        if (workers[i].buf_len + n < (int)sizeof(workers[i].buffer)) {
...
int scanner_check(const char *config_path) {
    ...
    if (zip[0]) fetch_and_process(zip, master_scan_file);
    else fetch_and_process("00000", master_scan_file);
